...
export interface AnchorRecovery {
  status: "exact" | "nearby" | "context" | "fuzzy" | "outdated";
  range?: DemoRange;
}
...
function sceneIndex(scene: DemoScene): number {
  return demoScenes.indexOf(scene);
}

function sceneAtLeast(scene: DemoScene, target: DemoScene): boolean {
  return sceneIndex(scene) >= sceneIndex(target);
}

function cloneProposal(proposal: EditProposal, status = proposal.status): EditProposal {
  return {
    ...proposal,
    status,
    target: {
...
function cloneFinding(finding: Finding, state = finding.state): Finding {
  return {
    ...finding,
    state,
...
function buildReviewThreads(scene: DemoScene): ReviewThread[] {
  if (!sceneAtLeast(scene, "review-arrives")) return [];
...
function buildHistory(scene: DemoScene): DemoHistoryEntry[] {
...
function getMainSourceForScene(scene: DemoScene): string {
  if (scene === "initial") return mainSourceInitial;
  if (scene === "writing") return mainSourceWriting;
  if (scene === "completion") return mainSourceWriting;
  if (scene === "compiler-error") return mainSourceCompilerError;
  if (sceneAtLeast(scene, "revision-accepted")) return mainSourceRevised;
  return mainSourceAccepted;
}

function getSourceForFile(file: string, scene: DemoScene): string {
  if (file === "main.tex") return getMainSourceForScene(scene);
  return demoFileSources[file] ?? demoFileSources["main.tex"] ?? mainSourceAccepted;
}

function defaultStageForScene(scene: DemoScene): DemoStage {
  return sceneMeta[scene].stage;
}

function buildSceneState(
  scene: DemoScene,
  previous?: ProductDemoState,
  overrides: Partial<Pick<ProductDemoState, "activeFile" | "activeStage">> = {},
): ProductDemoState {
  ...
  const diagnostics =
    scene === "compiler-error"
      ? [cloneFinding(compilerDiagnostic) as DemoDiagnostic]
      : [];
  ...
  return {
    ...
    editor: {
      ...
      selection:
        scene === "review-open" || proposalVisible
          ? { ...claimAnchor.range }
          : scene === "compiler-error"
            ? { ...figureAnchor.range }
            : undefined,
      highlightedRange:
        scene === "compiled" || sceneAtLeast(scene, "review-arrives")
          ? { ...claimAnchor.range }
          : undefined,
    ...
    diagnostics,
    suggestion:
      proposalVisible || proposalAccepted
...
export function createDemoState(reducedMotion = false): ProductDemoState {
  const scene: DemoScene = reducedMotion ? "complete" : "initial";
...
export function demoReducer(state: ProductDemoState, event: DemoEvent): ProductDemoState {
  switch (event.type) {
    case "START":
      if (state.playback.userInterrupted) return state;
...
export function getUnapprovedSceneCapabilities(): string[] {
...
function rangeForLineMatch(lineNumber: number, line: string, selectedText: string): DemoRange | undefined {
...
export function recoverTextAnchor(source: string, anchor: TextAnchor): AnchorRecovery {
  ...
  if (exactLine) {
    ...
    if (exactRange) return { status: "exact", range: exactRange };
  }
  ...
  for (let lineNumber = nearbyStart; lineNumber <= nearbyEnd; lineNumber += 1) {
    ...
    if (range) return { status: "nearby", range };
  }
  ...
  let best: AnchorRecovery | null = null;
  let bestScore = 0;
