...
const siteIconPaths = {
    ...
    Association: `<circle cx="12" cy="12" r="9" /><path d="M12 11v5" /><path d="M12 8h.01" />`,
    ...
    "Association / Legal Notice": `<circle cx="12" cy="12" r="9" /><path d="M12 11v5" /><path d="M12 8h.01" />`,
...
function initProductTeaser() {
    const teaser = query("[data-product-teaser]");
    if (!teaser)
        return;
    const shell = teaser.closest(".product-preview-shell") ?? document.body;
    const fileButtons = queryAll("[data-teaser-file]");
    const modeButtons = queryAll("[data-teaser-mode]");
    const annotationCards = Array.from(shell.querySelectorAll("[data-teaser-annotation]"));
    const notes = Array.from(shell.querySelectorAll("[data-teaser-notes] article"));
    const code = shell.querySelector("[data-teaser-code]");
    const tab = shell.querySelector("[data-teaser-tab]");
    const location = shell.querySelector("[data-teaser-location]");
    const paperTitle = shell.querySelector("[data-teaser-paper-title]");
    const paperCopy = shell.querySelector("[data-teaser-paper-copy]");
    const status = shell.querySelector("[data-teaser-status]");
    const compile = shell.querySelector("[data-teaser-compile]");
    const files = {
        main: {
            tab: "main.tex",
...
source section and the compiled PDF visible.`,
    ...
    function setFile(name) {
        ...
        if (paperTitle)
            paperTitle.textContent = file.title;
        if (paperCopy)
            paperCopy.textContent = file.copy;
...
async function initFooter() {
    ...
    queryAll(".site-footer nav a, .footer-static-link").forEach((link) => {
        const label = link.textContent?.trim() ?? "";
        const icon = getSiteIconPath(label);
