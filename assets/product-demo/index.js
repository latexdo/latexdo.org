import { aiTiers, claimAnchor, demoCapabilities, productRoutes, publicSceneControls, sceneCapabilities, sceneMeta, } from "./data.js";
import { createDemoState, demoReducer, demoTimeline, getTimelineDuration, } from "./engine.js";
const stageLabels = [
...
function trackDemoEvent(name, detail = {}) {
...
function getLineMarkerText(state) {
    if (state.scene === "compiler-error")
        return "\\includegraphics{results.pdf}";
    if (state.suggestion?.status === "accepted")
        return state.suggestion.after;
    if (state.suggestion)
        return state.suggestion.before;
    if (state.reviews.length)
        return claimAnchor.selectedText;
    return null;
}
function renderEditorLine(line, state) {
...
function renderReviewCard(state) {
...
function renderProblemPanel(state) {
...
function renderRebuttalPanel(state) {
...
function renderPaperReviewPanel(state) {
...
function renderSubmissionPanel(state) {
...
function renderInspector(state) {
    ...
    if (state.scene === "revision-proposed")
        return renderProposalPanel(state);
    if (state.scene === "rebuttal")
        return renderRebuttalPanel(state);
    if (state.scene === "paper-review")
        return renderPaperReviewPanel(state);
    if (state.scene === "submission-check" || state.scene === "complete")
        return renderSubmissionPanel(state);
    if (state.scene === "review-arrives" || state.scene === "review-open" || state.scene === "revision-accepted") {
        return renderReviewCard(state);
    }
    return renderProblemPanel(state);
}
function renderCapabilityRegistry(state) {
...
function findSceneTimelineOffset(scene) {
    const item = [...demoTimeline].reverse().find((timelineItem) => {
        const event = timelineItem.event;
        if (event.type === "SET_SCENE")
            return event.scene === scene;
        if (event.type === "SHOW_COMPLETION")
            return scene === "completion";
        if (event.type === "START_COMPILE")
            return scene === "compiling";
        if (event.type === "COMPILE_SUCCESS")
            return scene === "compiled";
        if (event.type === "OPEN_REVIEW")
            return scene === "review-open";
        if (event.type === "SHOW_PROPOSAL")
            return scene === "revision-proposed";
        if (event.type === "ACCEPT_PROPOSAL")
            return scene === "revision-accepted";
        if (event.type === "OPEN_REBUTTAL")
            return scene === "rebuttal";
        if (event.type === "RUN_PAPER_REVIEW")
            return scene === "paper-review";
        if (event.type === "RUN_SUBMISSION_CHECK")
            return scene === "submission-check";
        if (event.type === "AUTOPLAY_COMPLETE")
...
export function initProductDemo() {
    const tierContainer = document.querySelector("[data-ai-tier-cards]");
    if (tierContainer)
        tierContainer.innerHTML = renderAiTierCards();
    const routeContainer = document.querySelector("[data-product-route-links]");
    if (routeContainer)
        routeContainer.innerHTML = renderProductRouteLinks();
    ...
    const dispatch = (event) => {
        ...
        if (event.type === "SHOW_PROPOSAL")
            trackDemoEvent("demo_proposal_opened");
        if (event.type === "ACCEPT_PROPOSAL")
            trackDemoEvent("demo_proposal_accepted");
        if (event.type === "OPEN_REBUTTAL")
            trackDemoEvent("demo_rebuttal_opened");
        if (event.type === "RUN_SUBMISSION_CHECK")
            trackDemoEvent("demo_submission_opened");
        if (event.type === "AUTOPLAY_COMPLETE")
            trackDemoEvent("demo_completed");
    };
    const runTimeline = (fromMs = 0) => {
        if (state.playback.reducedMotion || state.playback.userInterrupted)
            return;
        clearTimeline();
        ...
        demoTimeline
            ...
            const timer = window.setTimeout(() => {
                ...
                dispatch(item.event);
                if (item.event.type === "AUTOPLAY_COMPLETE")
                    clearTimeline();
            }, item.at - fromMs);
            timers.push(timer);
        });
    ...
    root.addEventListener("click", (event) => {
        ...
        const sceneButton = target.closest("[data-demo-scene-target]");
        const stageButton = target.closest("[data-demo-stage-target]");
        const fileButton = target.closest("[data-demo-file]");
        const actionButton = target.closest("[data-demo-action]");
        const pdfBlock = target.closest("[data-pdf-block]");
        const downloadLink = target.closest("[data-demo-download]");
        if (downloadLink) {
            trackDemoEvent("download_clicked", { origin: downloadLink.dataset.origin ?? "product_demo" });
            return;
        }
        if (sceneButton) {
            setSceneManually(sceneButton.dataset.demoSceneTarget);
            return;
        }
        if (stageButton) {
            registerUserInteraction();
            dispatch({ type: "SET_STAGE", stage: stageButton.dataset.demoStageTarget });
            return;
        }
        if (fileButton) {
            registerUserInteraction();
        ...
        if (action === "replay")
            replay();
        if (action === "show-proposal")
            dispatch({ type: "SHOW_PROPOSAL", id: "proposal-claim-precision" });
        if (action === "accept-proposal")
            dispatch({ type: "ACCEPT_PROPOSAL", id: actionButton.dataset.id ?? "proposal-claim-precision" });
        if (action === "resolve-review")
            dispatch({ type: "ACCEPT_PROPOSAL", id: "proposal-claim-precision" });
        if (action === "apply-fix")
            dispatch({ type: "APPLY_FIX", id: actionButton.dataset.id ?? "compiler-figure-not-found" });
        if (action === "run-submission")
            dispatch({ type: "RUN_SUBMISSION_CHECK" });
    });
    root.addEventListener("keydown", (event) => {
        ...
        if (event.key !== "ArrowRight" && event.key !== "ArrowLeft")
            return;
        const button = target.closest("[data-demo-scene-target]");
        if (!button)
            return;
        const buttons = Array.from(root.querySelectorAll("[data-demo-scene-target]"));
    ...
    const chapters = Array.from(document.querySelectorAll("[data-demo-chapter]"));
    if ("IntersectionObserver" in window && chapters.length) {
        const chapterObserver = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                if (!entry.isIntersecting || entry.intersectionRatio < 0.6)
                    return;
                if (state.playback.playing || manualSceneOverride)
                    return;
                const scene = entry.target.dataset.demoChapter;
                if (scene)
    ...
    if ("IntersectionObserver" in window) {
        const demoObserver = new IntersectionObserver(([entry]) => {
            if (!entry?.isIntersecting || entry.intersectionRatio < 0.65)
                return;
            if (autoplayStarted || state.playback.reducedMotion)
                return;
            autoplayStarted = true;
            runTimeline(0);
        }, { threshold: [0.65] });
    ...
    win.__latexdoProductDemo = {
        getState: () => state,
        setScene: (scene) => setSceneManually(scene),
        replay,
    };
    render();
}
export function renderAiTierCards() {
    return aiTiers
...
export function renderProductRouteLinks() {
